...
[핵심 판단 원칙]
1. 검색어의 핵심 의미를 먼저 파악하세요.
   예) "캐시 선물하기" → 핵심 의미: 캐시(포인트/크레딧)를 다른 사람에게 주는 기능
   예) "흡연 구역" → 핵심 의미: 담배/흡연/연기와 관련된 불만이나 요청
   예) "지도 기반 매치" → 핵심 의미: 지도에서 경기 위치를 확인하는 기능

2. 동의어·유사 표현까지 폭넓게 매칭하세요.
   예) "캐시 선물하기" ≈ "캐시 선물", "캐시 보내기", "친구에게 캐시", "선물 기능", "캐시 기프트"
   예) "흡연 구역" ≈ "담배 냄새", "흡연", "연기", "금연", "흡연부스"
   예) "매니저 불만" ≈ "매니저 때문에", "매니저가 문제", "매니저 태도"

3. 반드시 VOC 본문(message)을 직접 읽고 판단. ticket_id·날짜·경로는 무시.

4. 완전히 다른 주제의 VOC는 included: false.
   - "캐시 선물" 검색 시: 지각/밸런스/매니저 불만 VOC → false
   - "흡연" 검색 시: 결제/밸런스/매니저 VOC → false
   - 관련 없으면 0건이어도 괜찮습니다. 억지로 맞추지 마세요.

[relevance 기준]
- 95~100: 검색어 키워드·동의어가 VOC 본문에 직접 등장
- 75~94: 핵심 의미와 명확히 같은 주제
- 50~74: 간접적으로 관련
- 50 미만 → included: false

[summary]
- 실제 찾은 관련 VOC 건수와 핵심 공통점 1문장
- 0건이면 "없음"만 작성
